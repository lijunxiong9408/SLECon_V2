...
@SetupView(
    path      = "Management::NvImport",
    sortIndex = 0x410
)
public class NvImportSetting extends SettingPanel<NvImport> implements Page, LiftDataChangedListener {
    ...
    public NvImportSetting ( LiftConnectionBean connBean ) {
        super(connBean);
    ...
    protected String getPanelTitle () {
        return TEXT.getString( "NvImport.title" );
    }
	...
	protected LinkedHashMap<String, Class<? extends JPanel>> getNavigation() {
		...
		map.put(Dict.lookup("NvImport"), null);
		return map;
	}
	
    public void setHot () {
    	if(DownFileAttributeReset) {
    		NvImport.SetDownLoadedAttribute(0,"--");
    	}else {
    		NvImport.SetDownLoadedAttribute(status.getUpdateDataCount(),status.getUpdateDataMD5());
    		
    		if(NvImport.GetFileMD5().equals(status.getUpdateDataMD5())) {
				int isDelete = JOptionPane.showConfirmDialog(null, "File Download Successful !", "Notify", JOptionPane.YES_NO_OPTION);
				if(isDelete == JOptionPane.YES_OPTION){
					sendImportCommand((byte)0);
				}
    		}
    	}
    }
    ...
    private boolean submit () {
    	if(IsVerify) {
	        try {
	        	...
	        	NvImport.SetFileChooseEnable(false);
	...
	@Override
	public void onOK(NvImport panel) {
	...
	@Override
	public void onReset(NvImport panel) {
		// TODO Auto-generated method stub
		reset();
	}
	...
	class MyThread extends Thread{
		...
		public void run() {
			...
			final String Path = NvImport. GetFilePath();
			InputStream in = null;  
            try {  
                ...
                while ((byteread = in.read(tempbytes)) != -1) {
                	...
                	misc.importNvram(ret);
                	Thread.sleep(500);
                } 
                if(FileSize == bytesend) {
                	app.SetVerifyButton(true);
                	setOKButtonEnabled(true);
    	        	NvImport.SetFileChooseEnable(true);
                }
            } catch (Exception e1) {  
                e1.printStackTrace();  
            } finally {  
   ...
   public void sendImportCommand(byte Type) {
	   misc.nvImportConfirm(Type);
   }
}
