...
import slecon.setting.setup.io.OutputsA05.DigitalOutputBean;
import slecon.setting.setup.io.OutputsA05.RelayOutputsBean;
...
public class OutputsSettingA05 extends SettingPanel<OutputsA05> implements Page, LiftDataChangedListener {
     ...
     * Logger.
     */
    private static final Logger         logger           = LogManager.getLogger( OutputsSettingA05.class );
    ...
    public OutputsSettingA05 ( LiftConnectionBean connBean ) {
        super(connBean);
    ...
    @Override
    public void onOK(OutputsA05 panel) {
        synchronized ( mutex  ) {
            setEnabled(false);
            if ( submit() )
                solid = null;
    ...
    @Override
    public void onReset(OutputsA05 panel) {
    	synchronized ( mutex  ) {
		     setEnabled(false);
		     reset();
		     setEnabled(true);
    ...
    public void setHot() {
        ...
        try {
            final OutputsA05.RelayOutputsBean bean_RelayOutputsBean = new OutputsA05.RelayOutputsBean();
            final OutputsA05.DigitalOutputBean bean_DigitalOutputBean = new OutputsA05.DigitalOutputBean();
    ...
    public void updateStatus () {
        final HashMap<OutputPinA05, Boolean> result   = new HashMap<>();
        CrossBar                            crossbar = new CrossBar( mcsconfig.getCrossbar() );
        byte[]                              rawIO    = status.getMcsIO();
        for ( OutputPinA05 pin : OutputPinA05.values() ) {
