...
import slecon.setting.setup.io.OutputsA03.DigitalOutputBean;
import slecon.setting.setup.io.OutputsA03.RelayOutputsBean;
...
public class OutputsSettingA03 extends SettingPanel<OutputsA03> implements Page, LiftDataChangedListener {
     ...
     * Logger.
     */
    private static final Logger         logger           = LogManager.getLogger( OutputsSettingA03.class );
    ...
    public OutputsSettingA03 ( LiftConnectionBean connBean ) {
        super(connBean);
    ...
    @Override
    public void onOK(OutputsA03 panel) {
        synchronized ( mutex  ) {
            setEnabled(false);
            if ( submit() )
                solid = null;
    ...
    @Override
    public void onReset(OutputsA03 panel) {
    	synchronized ( mutex  ) {
		     setEnabled(false);
		     reset();
		     setEnabled(true);
    ...
    public void setHot() {
        ...
        try {
            final OutputsA03.RelayOutputsBean bean_RelayOutputsBean = new OutputsA03.RelayOutputsBean();
            final OutputsA03.DigitalOutputBean bean_DigitalOutputBean = new OutputsA03.DigitalOutputBean();
    ...
    public void updateStatus () {
        final HashMap<OutputPinA03, Boolean> result   = new HashMap<>();
        CrossBar                            crossbar = new CrossBar( mcsconfig.getCrossbar() );
        byte[]                              rawIO    = status.getMcsIO();
        for ( OutputPinA03 pin : OutputPinA03.values() ) {
