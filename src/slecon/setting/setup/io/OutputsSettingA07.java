...
import slecon.setting.setup.io.OutputsA07.DigitalOutputBean;
import slecon.setting.setup.io.OutputsA07.RelayOutputsBean;
...
public class OutputsSettingA07 extends SettingPanel<OutputsA07> implements Page, LiftDataChangedListener {
     ...
     * Logger.
     */
    private static final Logger         logger           = LogManager.getLogger( OutputsSettingA07.class );
    ...
    public OutputsSettingA07 ( LiftConnectionBean connBean ) {
        super(connBean);
    ...
    @Override
    public void onOK(OutputsA07 panel) {
        synchronized ( mutex  ) {
            setEnabled(false);
            if ( submit() )
                solid = null;
    ...
    @Override
    public void onReset(OutputsA07 panel) {
    	synchronized ( mutex  ) {
		     setEnabled(false);
		     reset();
		     setEnabled(true);
    ...
    public void setHot() {
        ...
        try {
            final OutputsA07.RelayOutputsBean bean_RelayOutputsBean = new OutputsA07.RelayOutputsBean();
            final OutputsA07.DigitalOutputBean bean_DigitalOutputBean = new OutputsA07.DigitalOutputBean();
    ...
    public void updateStatus () {
        final HashMap<OutputPinA07, Boolean> result   = new HashMap<>();
        CrossBar                            crossbar = new CrossBar( mcsconfig.getCrossbar() );
        byte[]                              rawIO    = status.getMcsIO();
        for ( OutputPinA07 pin : OutputPinA07.values() ) {
