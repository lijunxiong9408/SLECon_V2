...
public class CarLinkViewPanel extends JPanel implements Page, LiftDataChangedListener{
    private static final long serialVersionUID = -552013171218549161L;
    static final ResourceBundle TEXT = ToolBox.getResourceBundle("inspect.CarDevice");
    ...
    private String[] navigationText = {Dict.lookup("Inspect"), Dict.lookup("Devices"), Dict.lookup("Car")};
    ///////////////////// interface <Page> /////////////////////
    public CarLinkViewPanel ( LiftConnectionBean bean) {
    ...
    protected void updateDeviceTable() {
        if (device != null) {
            ...
            byte[] devices = device.getAvailableDevcies(CANBus.CAR);
            if (devices != null) {
                for (Byte boardID : devices) {
                    String deviceType = device.getType(CANBus.CAR, boardID);
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                if (device != null && getSelectedDeviceId()!=null) {
                    ...
                    try {
                        type = DeviceType.valueOf(device.getType(CANBus.CAR, getSelectedDeviceId().byteValue()));
                    } catch (IllegalArgumentException|NullPointerException e) {}
                    if (type == null)
                        type = DeviceType.UNKNOWN;
                    info.setDeviceType(type);
                    info.setHarewareVersion(device.getHardwareVersion(CANBus.CAR, getSelectedDeviceId().byteValue()));
                    info.setFirmwareVersion(device.getFirmwareVersion(CANBus.CAR, getSelectedDeviceId().byteValue()));
                    
                    DisplayBean display    = new DisplayBean();
                    display.setPosition(device.getText(CANBus.CAR, getSelectedDeviceId().byteValue()));
                    display.setArrow(device.getArrow(CANBus.CAR, getSelectedDeviceId().byteValue()));
                    display.setMsg(device.getMessage(CANBus.CAR, getSelectedDeviceId().byteValue()));

                    int in=0, out=0, sync=0, blink=0, fast=0;  
                    for (byte i = 0; i < info.getDeviceType().ioCount; i++) {
                        final DeviceBehavior behavior = device.getBehavior(CANBus.CAR, getSelectedDeviceId().byteValue(), i);
                        ...
                        in    |= device.getInput(CANBus.CAR, getSelectedDeviceId().byteValue(), i) ? (1<<i) : 0;
                        out   |= device.getOutput(CANBus.CAR, getSelectedDeviceId().byteValue(), i) ? (1<<i) : 0;
                    }
