...
public enum InputPinA03 {
    ...
    LVCFB("LVCFB", 0x2e, false, ""),
    ENCF("ENCF", 0x49, true, ""),
    ADO("ADO", 0x1e, true, ""),
    V24MON("V24MON", 0x3f, true, ""),
    IOWD("IOWD", 0x6e, true, ""),
    ...
    DCS_LSL("DCS.LSL", 0x0e, false, ""),
    DCS_USL("DCS.USL", 0x1d, false, ""),
    ...
    private static final HashMap<Integer, InputPinA03> rawIOPosMap = new HashMap<Integer, InputPinA03>();
    private static final InputPinA03[] displayItem;



    static {
        List<InputPinA03> list = new ArrayList<>();
        for ( InputPinA03 is : values() ) {
            if(ToolBox.isDebugMode() || !is.debug) {
                rawIOPosMap.put( is.rawIOPos, is );
                list.add(is);
            }
        }
        displayItem = list.toArray(new InputPinA03[0]);
    }
    ...
    InputPinA03 ( String text, int rawIOPos, boolean debug, String firmwareVersion ) {
        this.text     = text;
    ...
    public static InputPinA03[] allDisplayItem() {
        return Arrays.copyOf(displayItem, displayItem.length);
    }

    
    public static InputPinA03[] allDisplayItem(String mcsFirmwareVersion) {
        List<InputPinA03> result = new ArrayList<>();
        AVersion v = ToolBox.parserVersion(mcsFirmwareVersion);
        for (InputPinA03 pin : displayItem) {
            if (v.compareTo(ToolBox.parserVersion(pin.firmwareVersion)) >= 0)
                result.add(pin);
        }
        return result.toArray(new InputPinA03[0]);
    }
    ...
    public static InputPinA03 getByRawIOPosition ( int rawIOPos ) {
