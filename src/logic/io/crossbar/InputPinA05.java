...
public enum InputPinA05 {
    ...
    private static HashMap<Integer, InputPinA05> rawIOPosMap = new HashMap<Integer, InputPinA05>();
    private static final InputPinA05[] displayItem;




    static {
        List<InputPinA05> list = new ArrayList<>();
        for ( InputPinA05 is : values() ) {
            if(ToolBox.isDebugMode() || !is.debug) {
                rawIOPosMap.put( is.rawIOPos, is );
                list.add(is);
            }
        }
        displayItem = list.toArray(new InputPinA05[0]);
    }
    ...
    InputPinA05 ( String text, int rawIOPos, boolean debug, String firmwareVersion ) {
        this.text     = text;
    ...
    public static InputPinA05[] allDisplayItem() {
        return Arrays.copyOf(displayItem, displayItem.length);
    }

    
    public static InputPinA05[] allDisplayItem(String mcsFirmwareVersion) {
        List<InputPinA05> result = new ArrayList<>();
        AVersion v = ToolBox.parserVersion(mcsFirmwareVersion);
        for (InputPinA05 pin : displayItem) {
            if (v.compareTo(ToolBox.parserVersion(pin.firmwareVersion)) >= 0)
                result.add(pin);
        }
        return result.toArray(new InputPinA05[0]);
    }
    ...
    public static InputPinA05 getByRawIOPosition ( int rawIOPos ) {
